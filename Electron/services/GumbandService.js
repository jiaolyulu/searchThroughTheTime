...
class GumbandService extends EventEmitter {
  ...
  async metrics(msg) {
    ...
    if (this.connected) await this.gb.event.create(eventName.toString(), value);
    logger.log("GUMBAND",`Metrics Event: ${eventName} with payload value ${JSON.stringify(value)}`);
}
