Class(function Wire() {
    ...
    (async function () {
        ...
        await Promise.all([
            initCurves('data/google/timeline/line_horizontal', _horizontal, 'horizontal'),
    ...
    const full = [];
        ...
        // run in parallel.
        await Promise.all(catmulls.map(async (chunk, chunkIndex) => {
    ...
    async function createLine(curve) {
        ...
        subdivisions = Math.floor(subdivisions * Tests.wireSubdivisionsMultiplier());
    ...
    function initConfig() {
        ...
        _config.setDescription('lerp', 'Lerp mapping between scroll and line drawing (0-1 value). Bigger the value, the line drawing will snap quickly towards the scroll progress.');

        _config.addNumber('speed', 1.0, 0.005);
        _config.setDescription('speed', 'Speed drawing relative to the camera progress');
    ...
    function loop() {
        ...
        _shader.uniforms.uExtraDrawing.value = Math.lerp(_extraDrawing, _shader.uniforms.uExtraDrawing.value, 0.01);
        ...
        _shader.uniforms.uErasing.value = Math.lerp(_shader.uniforms.uErasing.value, targetErasing, 0.02);
    }
