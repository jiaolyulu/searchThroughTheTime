( async function () {
	...
	class TransformControls extends Base3D {
		...
		constructor( camera, domElement ) {
			...
			// this.add(Utils3D.createDebug(1, Utils.randomColor()))
				...
				Object.defineProperty( scope, propName, {
					...
					set: function ( value ) {
						...
						if ( propValue !== value ) {
							...
							scope[propName+'Changed']?.({
								type: propName,
								value
							});

							scope.onChange(_changeEvent);
		...
		updateMatrixWorld() {
			...
			if ( this.object !== undefined ) {
				...
				if ( this.object._parent === null ) {

					console.error( 'TransformControls: The attached 3D object must be a part of the scene graph.' );

				} else {

					this.object._parent.matrixWorld.decompose( this._parentPosition, this._parentQuaternion, this._parentScale );
				...
				this._worldQuaternionInv.copy( this.worldQuaternion ).inverse();
		...
		pointerDown( pointer ) {
			...
			if ( this.axis !== null ) {
				...
				if ( planeIntersect ) {
					...
					this.object.updateMatrixWorld();
					this.object._parent.updateMatrixWorld();
				...
				this.onMouseDown(_mouseDownEvent);
		...
		pointerMove( pointer ) {
			...
			if ( mode === 'translate' ) {
				...
				if ( this.translationSnap ) {
					...
					if ( space === 'local' ) {
						...
						object.position.applyQuaternion( _tempQuaternion.copy( this._quaternionStart ).inverse() );
					...
					if ( space === 'world' ) {
						...
						if ( object._parent ) {
							...
							object.position.add( _tempVector.setFromMatrixPosition( object._parent.matrixWorld ) );
						...
						if ( object._parent ) {
							...
							object.position.sub( _tempVector.setFromMatrixPosition( object._parent.matrixWorld ) );
			...
			this.onChange(_changeEvent);
			this.onObjectChange(_objectChangeEvent);
		...
		dispose() {
			...
			this.traverse( function ( child ) {
				...
				if ( child.shader ) child.shader.dispose();
	...
	function intersectObjectWithRay( object, raycaster, includeInvisible ) {
		...
		intersectCache.length = 0;
		const allIntersections = raycaster.intersectObject( object, true, intersectCache, true );
	...
	Shader.prototype.setOpacity = function(value) {
		this.opacity = value;
		this.uniforms.uAlpha.value = value;
	}

	function Material(obj) {
		let shader = new Shader('ColorMaterial', Utils.mergeObject(obj, {
			uAlpha: {value: 1},
			color: {value: new Color(Utils.randomColor())}
		}));

		shader.color = shader.uniforms.color.value;

		shader.clone = function() {
			let clone = Material(obj);
			clone.uniforms.uAlpha.value = shader.uniforms.uAlpha.value;
			clone.uniforms.color.value.copy(shader.uniforms.color.value);
			return clone;
		}
	...
	class TransformControlsGizmo extends Base3D {
		...
		constructor() {
			...
			const matInvisible = Material({
				depthTest: false,
				depthWrite: false,
				transparent: true
			});
			matInvisible.setOpacity(0.15);
			const matHelper = Material({
				depthTest: false,
				depthWrite: false,
				transparent: true
			});
			matHelper.setOpacity(0.5);
			const matRed = Material({
				depthTest: false,
				depthWrite: false,
				transparent: true
			});
			matRed.uniforms.color.value.setHex(0xff0000);
			const matGreen = Material({
				depthTest: false,
				depthWrite: false,
				transparent: true
			});
			matGreen.uniforms.color.value.setHex( 0x00ff00 );
			const matBlue = Material({
				depthTest: false,
				depthWrite: false,
				transparent: true
			});
			matBlue.uniforms.color.value.setHex( 0x0000ff );
			const matRedTransparent = Material({
				depthTest: false,
				depthWrite: false,
				transparent: true
			});
			matRedTransparent.uniforms.color.value.setHex( 0xff0000 );
			matRedTransparent.setOpacity(0.5);
			const matGreenTransparent = Material({
				depthTest: false,
				depthWrite: false,
				transparent: true
			});
			matGreenTransparent.uniforms.color.value.setHex( 0x00ff00 );
			matGreenTransparent.setOpacity(0.5);
			const matBlueTransparent = Material({
				depthTest: false,
				depthWrite: false,
				transparent: true
			});
			matBlueTransparent.uniforms.color.value.setHex( 0x0000ff );
			matBlueTransparent.setOpacity(0.5);
			const matWhiteTransparent = Material({
				depthTest: false,
				depthWrite: false,
				transparent: true
			});
			matWhiteTransparent.setOpacity(0.25);
			const matYellowTransparent = Material({
				depthTest: false,
				depthWrite: false,
				transparent: true
			});
			matYellowTransparent.uniforms.color.value.setHex( 0xffff00 );
			matYellowTransparent.setOpacity(0.25);
			const matYellow = Material({
				depthTest: false,
				depthWrite: false,
				transparent: true
			});
			matYellow.uniforms.color.value.setHex(0xffff00);
			const matGray = Material({
				depthTest: false,
				depthWrite: false,
				transparent: true
			});
			matGray.uniforms.color.value.setHex( 0x999999 ); // reusable geometry

			const arrowGeometry = new CylinderGeometry( 0, 0.04, 0.1, 12 );
			arrowGeometry.applyMatrix(new Matrix4().makeTranslation( 0, 0.05, 0 ));
			const scaleHandleGeometry = new BoxGeometry( 0.08, 0.08, 0.08 );
			scaleHandleGeometry.applyMatrix(new Matrix4().makeTranslation( 0, 0.04, 0 ));
			const lineGeometry = new Geometry();
			lineGeometry.addAttribute( 'position', new GeometryAttribute( new Float32Array([ 0, 0, 0, 1, 0, 0 ]), 3 ) );
			const lineGeometry2 = new CylinderGeometry( 0.0075, 0.0075, 0.5, 3 );
			lineGeometry2.applyMatrix(new Matrix4().makeTranslation( 0, 0.25, 0 ));

			function CircleGeometry( radius, arc ) {

				const geometry = new TorusKnotGeometry( radius, 0.0075, 3, 64, arc * Math.PI * 2 );
				geometry.applyMatrix(new Matrix4().makeRotationY( Math.PI / 2 ));
			...
			function TranslateHelperGeometry() {
				...
				geometry.addAttribute( 'position', new GeometryAttribute( new Float32Array([ 0, 0, 0, 1, 1, 1 ]), 3 ) );
				return geometry;
			...
			const pickerRotate = {
				...
				X: [[ new Mesh( new TorusKnotGeometry( 0.5, 0.1, 4, 24 ), matInvisible ), [ 0, 0, 0 ], [ 0, - Math.PI / 2, - Math.PI / 2 ]]],
				Y: [[ new Mesh( new TorusKnotGeometry( 0.5, 0.1, 4, 24 ), matInvisible ), [ 0, 0, 0 ], [ Math.PI / 2, 0, 0 ]]],
				Z: [[ new Mesh( new TorusKnotGeometry( 0.5, 0.1, 4, 24 ), matInvisible ), [ 0, 0, 0 ], [ 0, 0, - Math.PI / 2 ]]],
				E: [[ new Mesh( new TorusKnotGeometry( 0.75, 0.1, 2, 24 ), matInvisible ) ]]
			};
			const gizmoScale = {
			...
			for ( let i = 0; i < handles.length; i ++ ) {
				...
				if ( handle.tag === 'helper' ) {
					...
					if ( handle.name === 'AXIS' ) {
						...
						_tempVector.applyQuaternion( this.worldQuaternionStart.clone().inverse() );
				...
				if ( this.mode === 'translate' || this.mode === 'scale' ) {
					...
					_alignVector.copy( this.eye ).applyQuaternion( _tempQuaternion.copy( quaternion ).inverse() );
				...
				handle.shader._color = handle.shader._color || handle.shader.color.clone();
				handle.shader._opacity = handle.shader._opacity || handle.shader.opacity;
				handle.shader.color.copy( handle.shader._color );
				handle.shader.setOpacity(handle.shader._opacity);

				if ( this.enabled && this.axis ) {

					if ( handle.name === this.axis ) {

						handle.shader.color.setHex( 0xffff00 );
						handle.shader.setOpacity(1.0);
	...
	class TransformControlsPlane extends Mesh {
		...
		constructor() {
			...
			super( new PlaneGeometry( 100000, 100000, 2, 2 ), Material() );
			this.type = 'TransformControlsPlane';

			this.shader.neverRender = true;
