Class(function MouseMilestones(_milestones) {
  ...
  function initConfig() {
    ...
    window.addEventListener("ToolTipOpenEvent", (e) => {
      onToolTipOpen(e);
    });
  }
  ...
  function sortByDistance(a, b) {
    let compare =
      distanceToCenter(a.screenPosition) - distanceToCenter(b.screenPosition);
    return compare;
  }
  ...
  function openCenterMostMilestone() {
    ...
    if (openMilestones.length > 0) {
      ...
      if (
        ...
        if (_currentOpenTooltip.shouldBeVisible && scroll-trigger<0) {
  ...
  function loopMilestone(m) {
    ...
    let force = Math.map(
      distance,
      _config.getNumber("minDistance"),
      _config.getNumber("maxDistance"),
      1,
      0.0,
      true
    );
